...
# Loading data files
wind_data = pd.read_excel("dayfldmean_anginv_850.xlsx")
event_data = pd.read_excel("angin_v_per_kategori.xlsx", sheet_name=None)

# Converting wind data dates to datetime
wind_data['valid_time'] = wind_data['valid_time'].apply(parse_date)
wind_data = wind_data.dropna(subset=['valid_time'])

# Preparing output Excel writer
output_file = "angin_per_kejadian.xlsx"
writer = pd.ExcelWriter(output_file, engine='xlsxwriter')

# Processing each event category (Lemah, Sedang, Kuat)
for category, df in event_data.items():
    # Converting event dates to datetime
    df['valid_time'] = df['valid_time'].apply(parse_date)
    df = df.dropna(subset=['valid_time'])

    # Iterating through each event
    for event_date in df['valid_time']:
        # Calculating h-2 to h+5 date range
        start_date = event_date - datetime.timedelta(days=2)
        end_date = event_date + datetime.timedelta(days=5)
        
        # Extracting year and month for sheet naming
        sheet_name = event_date.strftime("%Y%m")
        
        # Filtering wind data for the date range
        mask = (wind_data['valid_time'] >= start_date) & (wind_data['valid_time'] <= end_date)
        event_wind_data = wind_data.loc[mask, ['valid_time', 'v']].copy()
        
        # Ensuring data is sorted by date
        event_wind_data = event_wind_data.sort_values('valid_time')
        
        # Cleaning data: ensuring 'v' is numeric
        event_wind_data['v'] = pd.to_numeric(event_wind_data['v'], errors='coerce')
        event_wind_data = event_wind_data.dropna(subset=['v'])
        
        # Adding category column
        event_wind_data['category'] = category
        
        # Writing to Excel sheet (append if sheet exists)
        if sheet_name in writer.sheets:
            # If sheet exists, append data
            existing_data = pd.read_excel(output_file, sheet_name=sheet_name)
            combined_data = pd.concat([existing_data, event_wind_data], ignore_index=True)
            combined_data.to_excel(writer, sheet_name=sheet_name, index=False)
        else:
            # Create new sheet
            event_wind_data.to_excel(writer, sheet_name=sheet_name, index=False)

# Saving the Excel file
writer.close()
print(f"Processed wind data saved to {output_file}")
